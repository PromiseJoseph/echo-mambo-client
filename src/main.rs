...
#[tokio::main]
async fn main() {
    let client_addr_as_arg = std::env::args().nth(1);

    println!("Starting EchoMamboClient on");

    let listener: TcpListener =
        TcpListener::bind(client_addr_as_arg.unwrap_or(ECHOMAMBO_CLIENT_ADDR.to_string()))
            .await
            .expect("Failed to bind to address");

    println!(
        "EchoMamboClient is listening for incoming on {}",
...
async fn handle_client(mut stream: TcpStream) {
    ...
    loop {
        ...
        stream
            .write_all(echomambo_message.as_bytes())
            .await
            .unwrap(); // Send the response back to the user client
